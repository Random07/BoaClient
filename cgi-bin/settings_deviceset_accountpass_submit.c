...
int main()
{
    ...
    char TempUserPass1[30];
    char TempUserPass2[30];
    char TempUserPass3[30];
    char SendString[150];
    char SetResult[2];
...
//get_index_str_from_web_new(char *org,int index,char *Tag,char *outcome)
    ...
    if(!get_index_str_from_web(StringFromWeb,"textfield22=",TempUserPass1))
    {
        debug_message_printf("Can't find TempUserPass1");
        return 0;
    }
    
    if(!get_index_str_from_web(StringFromWeb,"textfield23=",TempUserPass2))
    {
        debug_message_printf("Can't find TempUserPass2");
        return 0;
    }

    if(!get_index_str_from_web(StringFromWeb,"textfield24=",TempUserPass3))
    {
        debug_message_printf("Can't find TempUserPass3");
        return 0;
    }

    if (strcmp(TempUserPass1,PassInDevice))
    {
        wifi_pro_alert_info="The Current password error,please retry!";
    ...
    if (!strcmp(TempUserPass2,TempUserPass3) && strlen(TempUserPass3) > 5)
    {
        strcat(SendString,NameInDevice);
        strcat(SendString,"|");
        strcat(SendString,TempUserPass3);
