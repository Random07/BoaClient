...
#ifdef Jinfei_LogTag
       JinfeiLog("Jinfei---COMMON_CGI_COUNT not high enough");
#endif
        exit(1);
    }
...
#ifdef Jinfei_LogTag
       JinfeiLog("Jinfei---Unable to allocate a component of common_cgi_env - out of memory");
#endif
            exit(1);
        }
    }
#ifdef Jinfei_LogTag
		   JinfeiLog("Jinfei---create_common_env---out");
#endif
...
#ifdef Jinfei_LogTag
	JinfeiLog(req->local_ip_addr);
	JinfeiLog(req->http_version);
	JinfeiLog(req->request_uri);
	JinfeiLog(req->path_info);
	JinfeiLog(req->path_translated);
	JinfeiLog(req->script_name);
	JinfeiLog(req->query_string);
	JinfeiLog(req->remote_ip_addr);
	JinfeiLog(simple_itoa(req->remote_port));
	JinfeiLog(req->content_type);
	JinfeiLog(req->content_length);
#endif
...
int init_cgi(request * req)
{
    ...
    case 0:
#ifdef Jinfei_LogTag
	JinfeiLog("Jinfei:fork return 0");
#endif
        /* child */
