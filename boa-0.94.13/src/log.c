...
#ifdef Jinfei_LogTag
void Jinfei_log_error_mesg(char *file, int line, char *function,char *mesg)
{
    int errno_save = errno;
    fprintf(stderr, "%s%s function:%s line:%d ", get_commonlog_time(), file, function, line);
    errno = errno_save;
    perror(mesg);
    errno = errno_save;
}
...
#ifdef Jinfei_LogTag
void Jinfei_log_to_access_log_mesg(char *file, int line, char *function,char *mesg)
{
	if (access_log) {
			    fprintf(access_log, "%s%s function:%s line:%d ---%s \n", get_commonlog_time(), file, function, line,mesg);
		}
}
#endif
